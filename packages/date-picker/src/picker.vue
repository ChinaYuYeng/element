...
const NewPopper = {
  ...
  data() {
    //这里是主要的区别，还有就是舍弃了一部分原来popper中的其他东西
...
const TYPE_VALUE_RESOLVER_MAP = {
  ...
  week: {
    ...
    formatter(value, format) {
      let week = getWeekNumber(value);//第几周
      let month = value.getMonth();//第几月
      const trueDate = new Date(value);
      if (week === 1 && month === 11) {
        trueDate.setHours(0, 0, 0, 0); //设置小时 0点0分0秒0毫秒
        trueDate.setDate(trueDate.getDate() + 3 - (trueDate.getDay() + 6) % 7);//设置日期
      }
      let date = formatDate(trueDate, format); //util 格式化
...
const valueEquals = function(a, b) {
  // considers Date object and string ，如果一个对象一个字符串不相等
...
const validator = function(val) {
  // either: String, Array of String, null / undefined ，任何以前类型
  return (
    val === null ||
    val === undefined ||
    isString(val) ||
...
//注入，基本和form配合使用
  ...
  created() {
    ...
    this.placement = PLACEMENT_MAP[this.align] || PLACEMENT_MAP.left;  //下拉的展示位置
  ...
  methods: {
    ...
    // {parse, formatTo} String deals with user input 字符串到时间对象
    parseString(value) {
...
//选择时间段的时候，鼠标操作时的值记录，第二次点击
    ...
    handleKeydown(event) {
      ...
      if (keyCode === 13) {
        if (this.userInput === '' || this.isValidValue(this.parseString(this.displayValue))) {
          this.handleChange();//更改值
          this.pickerVisible = this.picker.visible = false;//不显示
          this.blur();//失去焦点
        }
        event.stopPropagation();
        return;
  ...
  //隐藏picker
    ...
    //初始化picker ，并且，mounted元素
    mountPicker() {
      this.picker = new Vue(this.panel).$mount();//创建panel实例
      this.picker.defaultValue = this.defaultValue;
      this.picker.defaultTime = this.defaultTime;
      this.picker.popperClass = this.popperClass;
      this.popperElm = this.picker.$el;  //设置从元素
      this.picker.width = this.reference.getBoundingClientRect().width; //同主元素
