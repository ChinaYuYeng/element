...
TableStore.prototype.updateColumns = function() {
  ...
  const _columns = states._columns || []; //列的内部存储
  states.fixedColumns = _columns.filter((column) => column.fixed === true || column.fixed === 'left'); //拿到fixed的项（左侧）
  states.rightFixedColumns = _columns.filter((column) => column.fixed === 'right');//拿到右侧fixed项
  ...
  const notFixedColumns = _columns.filter(column => !column.fixed); //没fixed的项
  states.originColumns = [].concat(states.fixedColumns).concat(notFixedColumns).concat(states.rightFixedColumns); //原始的项
  ...
  states.columns = [].concat(fixedLeafColumns).concat(leafColumns).concat(rightFixedLeafColumns); //重新组装
