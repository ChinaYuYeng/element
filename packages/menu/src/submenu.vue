...
//选择性的使用了vue-popper中的一些属性作为mixins
  const poperMixins = {
    props: {
      transformOrigin: {
        type: [Boolean, String],
  ...
  export default {
    ...
    computed: {
      ...
      //菜单打开与否，当openedMenus改变之后，opened改变，菜单就会打开或者关闭，所以操作openedMenus增减就能操作开闭
      opened() {
      ...
      //是否悬浮块，横向菜单或者垂直折叠状态的菜单，子菜单使用悬浮块
      isMenuPopup() {
    ...
    methods: {
      ...
      handleCollapseToggle(value) {
        if (value) {
          this.initPopper();//展开浮动块
        } else {
          this.doDestroy();//销毁浮动块
        }
      },
      addItem(item) {
      ...
      handleClick() {
        ...
        this.dispatch('ElMenu', 'submenu-click', this);//通知父节submenu被点击
      },
      //鼠标进入弹出浮动块
      handleMouseenter() {
        const { rootMenu, disabled } = this;
        ...
        this.dispatch('ElSubmenu', 'mouse-enter-child');//告诉所有的子字节，鼠标进入
    ...
    created() {
      ...
      //监听当前菜单是否鼠标进入，并且清理当前快触发的原有操作
      this.$on('mouse-enter-child', () => {
        this.mouseInChild = true;
        clearTimeout(this.timeout);
      });
      //监听当前菜单是否鼠标移除，并且清理当前快触发的原有操作
