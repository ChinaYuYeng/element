<script>
  ...
  export default {
    ...
    mounted() {
      this.$on('menu-item-click', this.handleMenuItemClick);//监听下拉点击
      this.initEvent();
      this.initAria();
    },
    ...
    watch: {
      visible(val) {
        this.broadcast('ElDropdownMenu', 'visible', val); //广播通知menu打开或者关闭
        this.$emit('visible-change', val);
      },
      focusing(val) {
        const selfDefine = this.$el.querySelector('.el-dropdown-selfdefine');
        if (selfDefine) { // 自定义触发元素
          if (val) {
            selfDefine.className += ' focusing'; //增加聚焦样式
          } else {
            selfDefine.className = selfDefine.className.replace('focusing', '');//取消
          }
        }
      }
    },
    ...
    methods: {
      ...
      handleItemKeyDown(ev) {
        ...
        if ([38, 40].indexOf(keyCode) > -1) { // up/down
          ...
          this.removeTabindex();
          this.resetTabindex(this.menuItems[nextIndex]);//切换到下一个
          this.menuItems[nextIndex].focus();
          ev.preventDefault();
          ev.stopPropagation();
        } else if (keyCode === 13) { // enter选中
          this.triggerElm.focus();//聚集触发元素
          target.click();
      ...
      initAria() {
        ...
        this.menuItems = this.dropdownElm.querySelectorAll("[tabindex='-1']");//全是-1的
        this.menuItemsArray = Array.prototype.slice.call(this.menuItems);//转换成数组
      ...
      handleMenuItemClick(command, instance) {
        ...
        this.$emit('command', command, instance);//提供用户监听
      },
      //聚焦
      focus() {
